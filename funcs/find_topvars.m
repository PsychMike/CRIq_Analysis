...
for i = 1:size(stand_bins,1) %every subject
    ...
    %             wanted_col = j;
    %         end
    sub_var_scores(i,1) = abs(stand_bins(i,1)-(stand_bins(i,2)));
    %     end
end
...
for i = 1:size(top_sub_vars,1)
    for j = 1:size(top_sub_vars,2)
        ...
        elseif stand_bins(snum_index,two_comp) > stand_bins(snum_index,one_comp)
